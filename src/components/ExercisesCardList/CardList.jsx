...
export default function ExercisesCardList({ cardsPerPage, currentPage, activeTab, totalCards }) {
  return (
    <CardList>
      {[...Array(cardsPerPage)].map((_, index) => {
        ...
        if (cardNumber <= totalCards[activeTab]) {
          return (
            <ExerciseCard key={index + (currentPage - 1) * cardsPerPage} cardNumber={cardNumber} />
          );
        }
        return null;
      })}
